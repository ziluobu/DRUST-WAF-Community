<template>
    <div>
        <page-main>
            <el-button v-if="utils.hasPermission('webrules.store') || utils.hasPermission ('*.*')"
            ...
            <el-row :gutter="10" class="mb8">
                <el-button
                    v-if="utils.hasPermission('webrules.syncconfig') || utils.hasPermission ('*.*')"
                    type="primary"
                    plain
                    size="mini"
                    @click="handlConfig"
                ...
                <el-button v-if="utils.hasPermission('webrules.changestatus') || utils.hasPermission ('*.*')" size="small" plain
                           type="warning" :disabled="multiple1"
                           @click="multiOnchangeHandle"
                >
                    改变状态
                </el-button>
                <el-button v-if="utils.hasPermission('webrules.destroy') || utils.hasPermission ('*.*')" size="small" plain icon="el-icon-delete"
            ...
            >
                ...
                <el-table-column prop="web_id" label="网站" width="240" />
                <el-table-column prop="request_uri" label="请求url" width="240" />
                ...
                <el-table-column label="操作" width="160" fixed="right">
                    <template slot-scope="scope">
                        <el-button v-if="utils.hasPermission('webrules.update') || utils.hasPermission ('*.*')" type="primary" size="mini" plain @click="editHandle(scope.row)">编辑</el-button>
                        <el-button v-if="utils.hasPermission('webrules.show') || utils.hasPermission ('*.*')" type="warning" size="mini" plain @click="detailHandle(scope.row)">详情</el-button>
    ...
    methods: {
        ...
        handleDelete(row) {
            ...
            this.$confirm('是否确认删除该条' + '的数据项？?', '确认信息', {
                ...
                this.$api.delete(`api/webRules/${infoIds}`)
                    .then(res => {
                        if (res.code === 2000) {
                            this.initData()
                            this.$message({
        ...
        handleStatus(row) {
            ...
            //     type: 'warning'
            // }).then(() => {
            this.$api.put('api/webRules/changeStatus/', {
                id: infoIds,
        ...
        handlConfig() {
            this.$confirm('确定同步?', '提示', {
                ...
                this.loading = true
                this.$api.post('api/webRules/syncConfig')
                    .then(res => {
                        if (res.data) {
                            this.initData()
                            this.$message({
