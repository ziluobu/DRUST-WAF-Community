<template>
    <div style="height: 150px;">
        <div ref="attactDestAssetEcharts" style="width: 100%; height: 100%;" />
    </div>
</template>
<script>
...
export default {
    ...
    watch: {
        dataRank(old) {
            //
            this.attactDestAssetInfo2 = old.regionRank
            // arry.map(item => {
            //     this.attactDestAssetInfo2.push({
            //         name: item.name,
            //         value: item.total
            //     })
            // })
            if (this.attactDestAssetInfo2.length > 0) {

                this.$nextTick(function() {
                    this.drawAttactDestAssetEcharts()
                })
            }
    ...
    mounted() {
        // this.drawAttactDestAssetEcharts()
        if (this.dataRank.regionRank) {
            this.attactDestAssetInfo2 = this.dataRank.regionRank
            // arry.map(item => {
            //     this.attactDestAssetInfo2.push({
            //         name: item.name,
            //         value: item.total
    ...
    methods: {
        ...
        // 绘制攻击目的资产排名图表
        drawAttactDestAssetEcharts() {
            const $this = this
            if ($this.attactDestAssetInfo2.length > 0) {
                let max = Math.max.apply(Math, $this.attactDestAssetInfo2.map(function(o) { return o.value }))
                let unit = this.howMany(max).unit
                let dividend = this.howMany(max).value
                $this.attactDestAssetNoData = false
                let attackDestAssetEcharts = Echarts.init($this.$refs.attactDestAssetEcharts, null, { renderer: 'svg' })
                let option = $this.attactDestAssetOption
                let name = []
                let value = []
                let gapData = []
                for (let i = 0; i < $this.attactDestAssetInfo2.length; i++) {
                    name.push($this.attactDestAssetInfo2[i].name)
                    if (max > 1000) {
                        value.push({
                            unit: unit,
                            value: (parseInt($this.attactDestAssetInfo2[i].value) / dividend),
                            realValue: $this.attactDestAssetInfo2[i].value
                        })
                        gapData.push(max / dividend)
                    } else {
                        value.push({
                            unit: unit,
                            value: parseInt($this.attactDestAssetInfo2[i].value),
                            realValue: $this.attactDestAssetInfo2[i].value
                        })
                        gapData.push(max)
                    }
